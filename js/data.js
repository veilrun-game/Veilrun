...
VEILRUN.updates = [
  { date: "2026-08-02", title: "Arcline: two more levels (Temper + Citrine)", text: "Temper + Citrine now has three levels. The gate only opens once every trigger is powered. Level 2 needs two (throw a low blade, then a jump-throw for the high one). Level 3 puts a live-wire hazard across the path — Citrine has to spark it off before you can get through (that short zap of his finally earns its keep) — and then it's the two-trigger gate. Each keeps its own best-times board.", cta: { label: "Open the Lab →", href: "#lab" } },

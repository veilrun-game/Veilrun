...
VEILRUN.updates = [
  { date: "2026-08-02", title: "Arcline: two more levels (Temper + Citrine)", text: "Temper + Citrine now has three levels. Level 2 throws a live-wire hazard across the path — Citrine has to spark it off before you can pass (that short zap of his finally earns its keep) — then a two-trigger gate. Level 3 is the real test: plant three blades in a row across the relay pillars to route one long current all the way to the trigger. No single blade can span it and Citrine can't reach alone — you have to connect them all. Each keeps its own best-times board.", cta: { label: "Open the Lab →", href: "#lab" } },

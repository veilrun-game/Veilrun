...
VEILRUN.updates = [
  { date: "2026-09-02", games: ["arena-3d"], title: "Proving Ground: the strike lands", text: "**The strike had everything except a moment.** The numbers were right, the animation was trimmed to fit them back in August, and the screen still shook when you connected — but there was nothing that said *that one landed* at the instant it landed. **Now there is.**\n\n**A hit confirm, and it is deliberately not the crosshair.** The crosshair already turns magenta, and it always has — but that is a *prediction*: it means an Execute would land here, and it is on screen before you press anything. A confirm sharing an element with a prediction cannot be read as a confirm. **So there are two marks now and they mean different things:** the dot is what would happen, the flick of four strokes is what did. The finisher gets a bigger one, in magenta.\n\n**The lens kicks on every connect, not only on an Execute.** It is small — an Execute still pulls more than twice as hard, and that gap is the point. You should be able to feel which of the two just happened without looking at anything.\n\n**And there is a flash of light where the blade meets them**, with the blade itself brightening on the frame it lands.\n\n**The swing now draws what it actually hits, which it was not doing.** The finisher sweeps almost all the way round you — 290 degrees — and the arc on screen was drawing a 97-degree wedge for it, the same wedge as the light opener. **Three times too small, on the one attack that is supposed to catch everything.** The second hit was worse: its arc was drawn off to one side of where the damage was, so the picture and the hitbox disagreed. All three now draw their real cone, centred where the strike actually tests.\n\n**If you use reduced motion, the confirm still appears** — it just stops lunging at you. Damping is not allowed to cost you the answer to *did that land*, the same way the red flash when you get hit is dimmed rather than deleted.\n\n**One thing honestly did not get fixed: the finisher still does not look like a spin.** The mechanic sweeps all the way round; the animation is a stab. **That is missing art, not a missing number**, and no amount of extra shake would make it honest. It is next.\n\n**Nothing about the fight moved** — same damage, reach, cones, windows and leaderboard, checked automatically rather than promised. **Tell us whether it lands heavier now, and whether the confirm is too much on a phone.**", cta: { label: "\u25b6 Play Proving Ground", href: "#games/arena-3d" } },
  { date: "2026-08-31", games: ["arena-3d"], title: "Proving Ground: walk round and look at him", text: "**Yesterday\u0027s fix went half way.** Vesper stopped moonwalking, but in third person on a computer the mouse still turned *him* rather than the camera \u2014 which meant the camera could never get in front of him. **You could never look at your own character**, which is most of what a third-person camera is for.\n\n**Now the two are properly separate.** The stick or the keys move him and point him. The mouse or the right stick move the **camera**, and only the camera. Swing it right round and you are looking at his face while he keeps walking the way you are steering. Let go and he holds the way he was facing rather than snapping back.\n\n**First person is deliberately different, and stays as it was.** There you face where you look and move sideways \u2014 strafing, because that is what first person is for. Arcade is unchanged.\n\n**And it is a setting now, in the Camera group, per view.** \u2039Faces where you move\u203a is the new third-person default. \u2039Faces the camera\u203a is yesterday\u0027s behaviour, kept rather than deleted \u2014 it is a taste, and some people will want the mouse to turn him. \u2039Never turns\u203a strafes. You get one row and it is always the one for the view you are in.\n\n**Nothing about the fight moved** \u2014 same damage, reach, cones, windows and leaderboard, and that is checked automatically rather than promised. **Tell us whether third person feels right now, and whether you want the camera to drift back behind him on its own after a moment or stay exactly where you put it.**", cta: { label: "\u25b6 Play Proving Ground", href: "#games/arena-3d" } },
  { date: "2026-08-31", title: "The Loom folds up — and you can ask an idea for more", text: "**Three ideas at full height was most of a page.** The Loom shipped a week ago with every idea open all the way down — the pitch, four quoted takes, the gripes it was built around, who it fits — times three. Jordan put it plainly: *“I want each game to not take up so much space.”* He is right, and the fix was already sitting one panel below it: the game cards on this page learned to fold up two weeks ago, so now the ideas do too.\n\n**An idea is a title, its pitch, and its buttons.** Everything else is behind the line underneath — tap it and the whole thing opens where it stands. **Nothing is cut.** Every quote is still on the page whether it is open or not, so searching the page still finds it and a screen reader still reads it; only the space it takes up waits until you ask.\n\n**One thing deliberately did not get tucked away: who it came from.** The line you tap says **Built from 4 takes · Latch, Temper, Vesper** — because the whole reason to read this panel rather than skim past it is that every idea is quoting real people by name, and an idea folded down to a bare title would read like it came from nowhere. **The buttons stay out too.** A vote you have to open a card to cast is a vote nobody casts.\n\n**And there is a third button now: ✎ Tell me more.** Also Jordan's, and also right: *“some feel like they could use a bit of elaboration.”* Some of these are a sentence away from being something you could actually picture. **Press it and the ask is counted straight away** — then you get the option to say what specifically is thin, and you can close that without typing a word. The count still stands. **“This one needs more” is already the whole message**, and asking you to write a paragraph before it counts would mean it almost never gets sent.\n\n**It is not a vote.** It never moves the score, it can never dim an idea, and ▲/▼ are untouched — you can want an idea built *and* want more detail on it, which is exactly why the two are separate buttons. The number beside it is how many of you asked, and it is what the next batch reads when it decides which one to go deeper on.\n\n**Last thing, and it is overdue: “＋ Add a game” is at the top of the page now.** It used to be a panel underneath the Loom — the single thing this page exists to collect, parked below several screens of reading. **Six of you still aren't in a batch.** The button is the first thing on the page now, and it takes about a minute.", cta: { label: "Open the Game Reference", href: "#reference" } },

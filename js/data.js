    ...
    versions: [
      { id: "v1", label: "v1 (current)",
        controls: [
          ...
          ["Touch \u00b7 \u275a\u275a \u2699", "Top right \u2014 pause the arena, or open settings for camera view, how much arena fits the screen and look speed"]
        ],
        combos: [
...
VEILRUN.updates = [
  { date: "2026-08-22", games: ["arena-3d"], title: "The husks are bodies now — and we need one number off your phone", text: "**The husks have been flat pictures this whole time.** Every enemy in Proving Ground was a photograph of an enemy — a card that quietly turned to keep facing you. That is why twenty-six of them cost almost nothing, and it is also why they never quite felt like they were in the room with you. **They are real bodies now.** They run at you, they wind up and stab, their head snaps back when you catch them, and they fall over instead of shrinking. **The grey was never a placeholder — it was the answer.** The Thinned are written down as people drained hollow: grey, listless, one dying ember left. A featureless grey human shape with a single failing light in its chest is that description with nothing added to it, so that is exactly what they are. The ember doesn’t pulse steadily, either. Steady light is something you control; theirs flickers on two beats that never line up and drops out every so often, because it is the same light the crew carry, going out. **Their death got better by accident.** The falling animation is longer than the dissolve, so a husk now comes apart before it hits the floor — which is what ought to happen to something held together by a current that just went off. **And one thing we would rather be honest about than quietly get wrong: we do not know what this costs on your phone.** Twenty-six animated bodies is real work in a way twenty-six pictures never was, and every frame-rate reading this project has ever taken came from a browser tab that was not in front — which throttles itself and reports a number that means nothing. So the game starts careful: the husks nearest you are bodies, the ones further out stay cards, and it only promotes more of them once it has actually measured that there is room. You should not be able to see the join; if you can, tell us. **There is a Measure frame rate button in Settings.** It runs the same twenty-six-husk fight twice — once all bodies, once all cards — and tells you what the bodies cost, then hands you the number to paste back. It will refuse to give you a number at all if you looked away mid-run, which is the whole reason it exists. **If you play on a phone, that button is the most useful two minutes you can give us this week.** **No balance moved.** Same health, same damage, same reach, same windows, same leaderboard — the husks just stopped being pictures of themselves.", cta: { label: "▶ Play Proving Ground", href: "games/proving-ground/index.html" } },
